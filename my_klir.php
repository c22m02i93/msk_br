...
$status_get = $_GET['status_get'];
$alfa = $_GET['alfa'];
if ($blag_get == 'mon')  $blag_txt = '| Монастырь';
elseif ($blag_get) $blag_txt = '| '.$blag_get.' благочиние';

function status($string) {
	if ($string == 'штатный') $itog = 'green">ШТАТНЫЕ';
	if ($string == 'заштатный') $itog = 'orange">ЗАШТАТНЫЕ';
	if ($string == 'на покое') $itog = 'orange">НА ПОКОЕ';
	if ($string == 'архивный') $itog = 'orange">АРХИВНЫЕ';
	if ($string == 'запрещенный') $itog = 'red">ЗАПРЕЩЕННЫЕ';
return $itog;
}

?>
<title>Духовенство Барышской епархии<?php if ($blag_get) echo ' '.$blag_txt;  if ($status_get) { echo ' | ';
	if ($status_get == 'штатный') echo 'ШТАТНЫЕ';
	if ($status_get == 'заштатный') echo 'ЗАШТАТНЫЕ';
	if ($status_get == 'на покое') echo 'НА ПОКОЕ';
	if ($status_get == 'запрещенный') echo 'ЗАПРЕЩЕННЫЕ'; 
	if ($status_get == 'архивный') echo 'АРХИВНЫЕ'; 
}
?></title></head>
<body>
...
<?php
$stat_all = mysql_query("SELECT * FROM host1409556_barysh.klir");
echo 'Всего клириков: <b>'.mysql_num_rows($stat_all).'</b>';
$stat_all_presv = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE presv NOT LIKE ''");
echo ' (священников: <b>'.mysql_num_rows($stat_all_presv).'</b>';
$stat_all_diak = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE presv LIKE ''");
echo ', диаконов: <b>'.mysql_num_rows($stat_all_diak). '</b>)<br />';
$stat_all_stat = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'штатный'");
echo '&emsp; - штатных: <b>'.mysql_num_rows($stat_all_stat).'</b>';
$stat_all_stat_presv = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'штатный' AND presv NOT LIKE ''");
echo ' (свщ.: <b>'.mysql_num_rows($stat_all_stat_presv).'</b>';
$stat_all_stat_diak = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'штатный' AND presv LIKE ''");
echo ', диак.: <b>'.mysql_num_rows($stat_all_stat_diak). '</b>)<br />';
$stat_all_stat1 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'на покое'");
echo '&emsp; - на покое: <b>'.mysql_num_rows($stat_all_stat1).'</b>';
$stat_all_stat_presv1 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'на покое' AND presv NOT LIKE ''");
echo ' (свщ.: <b>'.mysql_num_rows($stat_all_stat_presv1).'</b>';
$stat_all_stat_diak1 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'на покое' AND presv LIKE ''");
echo ', диак.: <b>'.mysql_num_rows($stat_all_stat_diak1). '</b>)<br />';
$stat_all_stat2 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'заштатный'");
echo '&emsp; - заштатных: <b>'.mysql_num_rows($stat_all_stat2).'</b>';
$stat_all_stat_presv2 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'заштатный' AND presv NOT LIKE ''");
echo ' (свщ.: <b>'.mysql_num_rows($stat_all_stat_presv2).'</b>';
$stat_all_stat_diak2 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'заштатный' AND presv LIKE ''");
echo ', диак.: <b>'.mysql_num_rows($stat_all_stat_diak2). '</b>)<br />';
$stat_all_stat3 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'запрещенный'");
echo '&emsp; - запрещенных: <b>'.mysql_num_rows($stat_all_stat3).'</b>';
$stat_all_stat_presv3 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'запрещенный' AND presv NOT LIKE ''");
echo ' (свщ.: <b>'.mysql_num_rows($stat_all_stat_presv3).'</b>';
$stat_all_stat_diak3 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'запрещенный' AND presv LIKE ''");
echo ', диак.: <b>'.mysql_num_rows($stat_all_stat_diak3). '</b>)<br />';
$stat_all_stat4 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'архивный'");
echo '&emsp; - архивных: <b>'.mysql_num_rows($stat_all_stat4).'</b>';
$stat_all_stat_presv4 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'архивный' AND presv NOT LIKE ''");
echo ' (свщ.: <b>'.mysql_num_rows($stat_all_stat_presv4).'</b>';
$stat_all_stat_diak4 = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE 'архивный' AND presv LIKE ''");
echo ', диак.: <b>'.mysql_num_rows($stat_all_stat_diak4). '</b>)<br />';
...
if ($blag_get) {
		$klirik_all = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE blag LIKE '$blag_get' ORDER by FIELD(status, 'штатный', 'на покое', 'заштатный', 'запрещенный', 'архивный'), name ASC");

}
elseif ($alfa) {
		$klirik_all = mysql_query("SELECT * FROM host1409556_barysh.klir ORDER by name ASC");

}
elseif ($status_get) {
		$klirik_all = mysql_query("SELECT * FROM host1409556_barysh.klir WHERE status LIKE '$status_get' ORDER by FIELD(blag, '1', '2', '3', '4', '5', 'mon'), name ASC");

}
else {
		$klirik_all = mysql_query("SELECT * FROM host1409556_barysh.klir ORDER BY FIELD(status, 'штатный', 'на покое', 'заштатный', 'запрещенный', 'архивный'), FIELD(blag, '1', '2', '3', '4', '5', 'mon'), name ASC");
...
if (empty($alfa)) {
	if (empty($status) || ($status != $klirik['status'])) {
$status = $klirik['status'];
 echo '<br /><p style="color: '.status($status).'</p>';
	}
...
echo '</h3>';
$blag_all = mysql_query("SELECT * FROM host1409556_barysh.blagochiniya WHERE id LIKE '$blag' LIMIT 1");
...
else echo '<span class="photos"><img style="width: 95px; box-shadow: 2px 2px 5px rgba(0,0,0,0.3); display: inline;float: left;border: 1px solid #C3D7D4; margin: 0 10px 5px 10px; padding: 10px" src="IMG/no_foto.jpg" alt= "'.$klirik['san'].' '.$klirik['name'].'" title="'.$klirik['san'].' '.$klirik['name'].'"/></span>';
echo '<span class="title"><a href="my_klirik.php?id='.$klirik['id'].'"><b>'.$klirik['name'].'</b></a><span style="color:#666"> '.$klirik['san'].'</span></span><br /><br />';
